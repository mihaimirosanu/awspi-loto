...
```bash
awspi-loto$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
awspi-loto$ sam local start-api
awspi-loto$ curl http://localhost:3000/
```
...
```bash
awspi-loto$ sam logs -n HelloWorldFunction --stack-name awspi-loto --tail
```
...
```bash
awspi-loto$ cd HelloWorldFunction
...
```bash
sam delete --stack-name awspi-loto
```
